...
# Names that are places, not venues. The extractor is told to skip these; this is
...
# Latin generics need a word boundary; CJK ones must not have one. \b sits
# between a word and a non-word character, and every CJK glyph is a word
# character, so \b(酒家)\b never matches inside 适苑酒家 -- which silently split
# one restaurant into two venue rows.
